  ...
  namespace :load do
    ...
    task :geocode => :environment do
      Resume.all.each do |resume|
        if !resume.location && (resume.state || resume.zip)
        ...
        logger.info "#{resume.primary_email} #{resume.location}"
      end
    end
    ...
    def create_or_update(resume, resume_text, md5sum)
      ...
      if duplicate_resume = Resume.find_by(primary_email: get_primary_email(resume_text))
      ...
      Resume.create(
        ...
        primary_email:      get_primary_email(resume_text),
    ...
    def get_primary_email(full_text)
